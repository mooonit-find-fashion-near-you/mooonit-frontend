...
export function CartItem({ item, onQuantityChange, onRemove, updating = false }: CartItemProps) {
    ...
    return (
        <div className={`relative flex items-center gap-4 sm:gap-6 ${updating ? 'opacity-50' : ''}`}>
            ...
            <div className="flex-1">
                ...
                <h3 className="text-2xl font-semibold text-[#2c2d3a]">{item.name}</h3>
                <p className="text-sm text-[#757575]">Size: {item.size}</p>

                <p className="mt-2 text-lg font-semibold text-[#e54b4b] font-[TOPLUXURY]">
            ...
            {/* Delete */}
            <button
                className="text-[#e54b4b] disabled:opacity-50 absolute right-4 top-4 cursor-pointer"
